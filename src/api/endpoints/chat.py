...
from src.models.chat_models import ChatRequest, ChatResponse, StreamingChatChunk
...
async def get_rag_service(
    ...
    if _rag_service is None:
        ...
        try:
            ...
            logger.warning(
                f"Search index initialization failed, will retry on demand: {e}"
            )
    return _rag_service
...
async def chat_completion(
    request: ChatRequest,
    use_rag: bool = Query(
        False, description="Enable RAG (knowledge-enhanced) responses"
    ),
...
async def rag_chat_completion(
    request: ChatRequest, rag_service: RAGEnhancedAIService = Depends(get_rag_service)
) -> RAGResponse:
    """
...
async def chat_completion_stream(
    request: ChatRequest, ai_service: AzureOpenAIService = Depends(get_ai_service)
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to initialize streaming response"
        )
...
async def rag_chat_completion_stream(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to initialize streaming RAG response"
        )
...
async def search_knowledge_base(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to search knowledge base. Please try again."
        )
...
async def get_knowledge_base_stats(
    ...
    try:
        ...
        response = {
            ...
            "available_document_types": list(stats.documents_by_type.keys())
        ...
        logger.info(
            ...
            total_documents=stats.total_documents,
            document_types=len(stats.documents_by_type)
            if stats.documents_by_type
            else 0,
        )
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve knowledge base statistics"
        )


@router.get("/conversations/{conversation_id}/summary")
async def get_conversation_summary(
    conversation_id: str, ai_service: AzureOpenAIService = Depends(get_ai_service)
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve conversation summary"
        )
...
async def get_active_conversations(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve conversation statistics"
        )

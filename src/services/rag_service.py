...
from src.models.chat_models import ChatMessage, ChatRequest, ChatResponse
from src.models.rag_models import (
    SearchQuery, SearchResult, RAGResponse, DocumentType
)
...
class RAGEnhancedAIService:
    ...
    def __init__(self, settings: Settings):
        ...
        self.enable_rag_for_query_types = [
            "career", "skill", "interview", "salary", "learning", "transition"
        ]
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            if not index_created:
                logger.warning("Search index initialization failed, RAG features may be limited")
    ...
    def _should_use_rag(self, query: str, conversation_context: Optional[str] = None) -> bool:
        """
        ...
        career_keywords = [
            "career", "job", "skill", "interview", "salary", "learning", 
            "transition", "ai engineer", "machine learning", "data science",
            "resume", "portfolio", "experience", "qualification", "certification",
            "bootcamp", "degree", "course", "training", "mentor", "advice"
        ]
    ...
    async def generate_rag_response(self, request: ChatRequest) -> RAGResponse:
        ...
        try:
            ...
            if request.conversation_id:
                conversation_history = self.conversation_history.get(request.conversation_id, [])
            ...
            if use_rag:
                ...
                retrieved_sources = await self.search_service.semantic_search(search_query)
            ...
            if retrieved_sources:
                avg_similarity = sum(s.similarity_score for s in retrieved_sources) / len(retrieved_sources)
                confidence_score = min(0.95, 0.7 + (avg_similarity * 0.25))  # Scale to 0.7-0.95
            ...
            if request.conversation_id:
                ...
                if len(history) > self.max_history_messages:
                    self.conversation_history[request.conversation_id] = history[-self.max_history_messages:]

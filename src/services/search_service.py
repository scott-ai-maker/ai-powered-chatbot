...
from src.models.rag_models import (
    KnowledgeDocument, SearchQuery, SearchResult, RAGResponse,
    KnowledgeBaseStats, IndexingStatus, DocumentType
)
...
class AzureCognitiveSearchService:
    ...
    async def index_document(self, document: KnowledgeDocument) -> bool:
        ...
        try:
            ...
            if result[0].succeeded:
                ...
                logger.error(f"Failed to index document {document.id}: {result[0].error_message}")
                return False
    ...
    async def index_documents_batch(self, documents: List[KnowledgeDocument]) -> IndexingStatus:
        """
            ...
            for i in range(0, len(documents), batch_size):
                ...
                for document in batch:
                    try:
                        ...
                        status.error_messages.append(f"Document {document.id}: {str(e)}")
                        continue
                
                # Upload batch
                if batch_documents:
                    try:
                        results = await self.search_client.upload_documents(batch_documents)
    ...
    async def semantic_search(self, query: SearchQuery) -> List[SearchResult]:
        ...
        try:
            ...
            if query.document_types:
                type_filter = " or ".join([f"document_type eq '{dt.value}'" for dt in query.document_types])
            ...
            async for result in results:
                ...
                similarity_score = max(semantic_score / 4.0, vector_score)  # Normalize semantic score
            ...
            logger.info(f"Semantic search returned {len(search_results)} results for query: {query.query[:50]}...")

...
class TestFullApplicationFlow:
    ...
    async def test_health_check_integration(self, client, complete_environment):
        ...
        required_fields = [
            ...
            "azure_openai_status", "database_status"
        ]
    ...
    def test_request_validation_integration(self, client, complete_environment):
        ...
        invalid_requests = [
            ...
            {"message": "test", "user_id": "test", "temperature": 3.0},  # Invalid temperature
            {"message": "test", "user_id": "test", "max_tokens": 0},  # Invalid max_tokens
        ]
...
class TestApplicationStateManagement:
    ...
    async def test_service_initialization_on_startup(
        ...
        monkeypatch.setenv('AZURE_OPENAI_ENDPOINT', 'https://startup-test.openai.azure.com/')
    ...
    async def test_concurrent_requests_handling(self, client, monkeypatch):
        ...
        monkeypatch.setenv('AZURE_OPENAI_ENDPOINT', 'https://concurrent-test.openai.azure.com/')
    ...
    async def test_memory_usage_across_conversations(
        ...
        monkeypatch.setenv('AZURE_OPENAI_ENDPOINT', 'https://memory-test.openai.azure.com/')
...
class TestRealWorldScenarios:
    ...
    def setup_environment(self, monkeypatch):
        ...
        monkeypatch.setenv('AZURE_OPENAI_ENDPOINT', 'https://realistic-test.openai.azure.com/')
    ...
    async def test_multiple_users_concurrent_conversations(
        ...
        users = [
            {"user_id": "user_001", "conv_id": "conv_001", "topic": "AI career transition"},
            {"user_id": "user_002", "conv_id": "conv_002", "topic": "Machine learning skills"},
            {"user_id": "user_003", "conv_id": "conv_003", "topic": "Data science path"}
        ]
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
            ...
            all_responses = [future.result() for future in concurrent.futures.as_completed(futures)]
        ...
        for user_responses in all_responses:
            ...
            conv_ids = [response.json()["conversation_id"] for response in user_responses]

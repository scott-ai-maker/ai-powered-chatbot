...
class AsyncChatbotService:
    ...
    async def handle_conversation(self, user_id: str, messages: List[str]) -> List[ChatResponse]:
        """Handle multiple messages concurrently - demonstrates async power."""
        
        # Convert strings to validated ChatMessage objects
        chat_messages = [
            ChatMessage(content=msg, user_id=user_id) 
            for msg in messages
        ]
...
async def demonstrate_production_patterns():
    ...
    print(f"✅ Settings loaded: {settings.model_name} with {settings.max_concurrent_requests} max concurrent requests")
    ...
    async with AsyncChatbotService(settings) as chatbot:
        ...
        print(f"  • Messages processed concurrently: {settings.max_concurrent_requests}")
...
def explain_the_patterns():
    ...
    print(f"  • FastAPI endpoints will use these same async patterns")
    print(f"  • Azure OpenAI calls will be async HTTP requests")
    print(f"  • Pydantic models will validate all API requests/responses")
    print(f"  • Settings will load Azure credentials from environment")
    print(f"  • Multiple users can chat simultaneously without blocking")

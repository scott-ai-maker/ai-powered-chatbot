...
class MockChatbotClient:
    ...
    def _generate_demo_response(self, message: str) -> str:
        ...
        if any(word in message_lower for word in ["transition", "switch", "change career", "move to ai"]):
            return self.demo_responses["transition"]
        elif any(word in message_lower for word in ["skills", "learn", "technology", "programming"]):
            return self.demo_responses["skills"]  
        elif any(word in message_lower for word in ["salary", "pay", "money", "compensation", "earn"]):
